...
document.getElementById('meetupForm').addEventListener('submit', function(event) {
  ...
  var file = fileInput.files[0] ? fileInput.files[0].name : 'Nenhuma imagem selecionada';

  localStorage.setItem('meetupData', JSON.stringify({
      creator: creator,
  ...
  window.location.href = 'meetUpCriada.html';
});
});

  //TELA ASSISTIDOS E RECOMENDADOS (passar páginas)
document.addEventListener('DOMContentLoaded', () => {
  const pages = document.querySelectorAll('.pagination a.page');
  const prev = document.querySelector('.pagination a.prev');
  ...
  function updatePagination() {
      ...
      prev.parentElement.style.display = currentPage === 1 ? 'none' : 'inline';
      next.parentElement.style.display = currentPage === pages.length ? 'none' : 'inline';
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        const link = document.getElementeById('link').value.trim();
        const date = document.getElementById('date').value.trim();
        const time = document.getElementById('time').value.trim();

        if(photo && creator && description && link && dat && time){
        console.log('Foto Principal:', photo);
        ...
        alert("Meetup criada com sucesso!")
        } else{
  ...
  //TELA DETALHES MEETUP

  // favoritar /desfavoritar /copiar link
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializa o estado do botão de favorito
    ...
    favoriteButton.addEventListener('click', function() {
        ...
        // Atualiza o estado do botão após a mudança
        updateFavoriteButton();
    });

    shareButton.addEventListener('click', function() {
        // Adicione aqui a lógica para compartilhar o meetup
        alert('Link da meetup copiado.');
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var names = ["João", "Maria", "Pedro", "Ana", "Carlos", "Lúcia", "Fernando", "Camila", "Gabriel", "Laura"];

  // Lista de sobrenomes fictícios
  var lastNames = ["Silva", "Santos", "Oliveira", "Souza", "Pereira", "Ferreira", "Almeida", "Carvalho", "Gomes", "Martins"];

  // Lista de URLs de imagens de homens fictícios
  var maleAvatars = [
  ...
  // Lista de URLs de imagens de mulheres fictícias
  var femaleAvatars = [
  ...
  // Lista de URLs de avatares já utilizados
  var usedAvatars = [];
  ...
  document.querySelectorAll('.star').forEach(function(star) {
    star.addEventListener('click', function() {
      ...
      // Registra o valor clicado
      console.log('Valor selecionado:', clickedValue);
    });

    // Adiciona evento de passar o mouse sobre as estrelas
    star.addEventListener('mouseenter', function() {
      var hoveredValue = this.getAttribute('data-value');
      var stars = this.parentElement.querySelectorAll('.star');
      stars.forEach(function(s) {
        var starValue = s.getAttribute('data-value');
    ...
    // Adiciona evento de sair do mouse das estrelas
  ...
  document.getElementById('submit-comment').addEventListener('click', function() {
    ...
    // Verifica se o nome gerado é feminino ou masculino para selecionar o avatar correspondente
    if (randomNameIndex % 2 === 0) {
      // Seleciona aleatoriamente um avatar masculino que ainda não foi utilizado
      do {
        avatarURL = maleAvatars[Math.floor(Math.random() * maleAvatars.length)];
      } while (usedAvatars.includes(avatarURL));
    } else {
      // Seleciona aleatoriamente um avatar feminino que ainda não foi utilizado
      do {
    ...
    // Adiciona o avatar à lista de avatares utilizados
    usedAvatars.push(avatarURL);

    var userNameSpan = document.createElement('span');
    // Seleciona aleatoriamente um nome e sobrenome
    userNameSpan.textContent = names[randomNameIndex] + " " + lastNames[Math.floor(Math.random() * lastNames.length)];
    ...
    // Limpar o campo de texto após o envio do comentário
    document.getElementById('comment').value = '';

    // Remover a classe 'active' das estrelas de entrada de comentários
    document.querySelectorAll('.star').forEach(function(star) {
...
document.addEventListener("DOMContentLoaded", function() {
  document.getElementById("loginForm").addEventListener("submit", function(event) {
      ...
      if (email === "usuario@example.com" && senha === "senha123") {
          window.location.href = 'index-login.html'; // Redirect to 'index.html'
      } else {
